...
SELECT id, category_id, name, description, unlock_method, unlock_requirement_amount, required_achievement_id, required_quest_id, is_secret FROM achievements WHERE id = $1
`
...
func (q *Queries) GetAchievementById(ctx context.Context, id int32) (Achievement, error) {
	...
	err := row.Scan(
		...
		&i.Description,
		&i.UnlockMethod,
		&i.UnlockRequirementAmount,
		&i.RequiredAchievementID,
		&i.RequiredQuestID,
		&i.IsSecret,
	)
	return i, err
...
const getAllUserAchievements = `-- name: GetAllUserAchievements :many
SELECT uas.user_id, uas.achievement_id, uas.current_progress, uas.is_complete, uas.unlocked_at, uas.last_updated,
...
type GetAllUserAchievementsRow struct {
	UserID               uuid.UUID        `json:"user_id"`
	AchievementID        int32            `json:"achievement_id"`
	CurrentProgress      pgtype.Int4      `json:"current_progress"`
	...
	AchievementGroupName string           `json:"achievement_group_name"`
	AchievementName      string           `json:"achievement_name"`
}

// SELECT * FROM user_achievements WHERE user_id = $1;
func (q *Queries) GetAllUserAchievements(ctx context.Context, userID uuid.UUID) ([]GetAllUserAchievementsRow, error) {
	rows, err := q.db.Query(ctx, getAllUserAchievements, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetAllUserAchievementsRow
	for rows.Next() {
		var i GetAllUserAchievementsRow
		if err := rows.Scan(
			&i.UserID,
			&i.AchievementID,
			&i.CurrentProgress,
...
const getUserAchievementById = `-- name: GetUserAchievementById :one
SELECT user_id, achievement_id, current_progress, is_complete, unlocked_at, last_updated FROM user_achievements WHERE achievement_id = $1
`

func (q *Queries) GetUserAchievementById(ctx context.Context, achievementID int32) (UserAchievement, error) {
	row := q.db.QueryRow(ctx, getUserAchievementById, achievementID)
...
const getUserAchivementsByCategory = `-- name: GetUserAchivementsByCategory :many
SELECT ua.user_id, ua.achievement_id, ua.current_progress, ua.is_complete, ua.unlocked_at, ua.last_updated,
    a.name as achievement_name,
    a.category_id,
...
type GetUserAchivementsByCategoryParams struct {
	UserID  uuid.UUID   `json:"user_id"`
	Column2 interface{} `json:"column_2"`
}

type GetUserAchivementsByCategoryRow struct {
	UserID          uuid.UUID        `json:"user_id"`
	AchievementID   int32            `json:"achievement_id"`
	CurrentProgress pgtype.Int4      `json:"current_progress"`
	IsComplete      pgtype.Bool      `json:"is_complete"`
...
func (q *Queries) GetUserAchivementsByCategory(ctx context.Context, arg GetUserAchivementsByCategoryParams) ([]GetUserAchivementsByCategoryRow, error) {
	rows, err := q.db.Query(ctx, getUserAchivementsByCategory, arg.UserID, arg.Column2)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetUserAchivementsByCategoryRow
	for rows.Next() {
		var i GetUserAchivementsByCategoryRow
		if err := rows.Scan(
			&i.UserID,
			&i.AchievementID,
			&i.CurrentProgress,
...
const giveUserAchievement = `-- name: GiveUserAchievement :one
INSERT INTO user_achievements (user_id, achievement_id, current_progress, is_complete)
SELECT $1, a.id, 0, false
FROM achievements a
WHERE a.id = $2
RETURNING user_id, achievement_id, current_progress, is_complete, unlocked_at, last_updated
`

type GiveUserAchievementParams struct {
	UserID uuid.UUID `json:"user_id"`
	ID     int32     `json:"id"`
}

func (q *Queries) GiveUserAchievement(ctx context.Context, arg GiveUserAchievementParams) (UserAchievement, error) {
	row := q.db.QueryRow(ctx, giveUserAchievement, arg.UserID, arg.ID)
	var i UserAchievement
	err := row.Scan(
		&i.UserID,
		&i.AchievementID,
